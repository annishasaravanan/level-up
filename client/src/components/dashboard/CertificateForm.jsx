...
import CodingCertificateFields from '../certificates/CodingCertificateFields';
...
const CertificateForm = ({ selectedType, onSelectType, onClose }) => {
  ...
  const handleBackButton = () => {
    onSelectType('');  // Use empty string instead of null
  };
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 animate-scale-in overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            ...
            <input
              ...
              type="text"
              required
              placeholder={`${selectedType === 'event' ? 'Event' : selectedType === 'workshop' ? 'Workshop' : selectedType === 'coding' ? 'Challenge' : selectedType === 'project' ? 'Project' : 'Course'} title`}
          ...
          <div>
            <label htmlFor="issuer" className="form-label">Issuer/Provider*</label>
            <input
              id="issuer"
              name="issuer"
              type="text"
              required
              placeholder={`${selectedType === 'event' ? 'Event organizer' : selectedType === 'workshop' ? 'Workshop provider' : selectedType === 'coding' ? 'Platform name' : selectedType === 'project' ? 'Organization' : 'Institution name'}`}
              className="form-input"
        ...
        {selectedType === 'event' && (
          <EventCertificateFields formData={formData} handleChange={handleChange} />
        )}
        ...
        {selectedType === 'coding' && (
          <CodingCertificateFields formData={formData} handleChange={handleChange} />
        )}
        ...
        {selectedType === 'course' && (
          <CourseCertificateFields formData={formData} handleChange={handleChange} />
        )}
        
        <div className="border-t border-gray-100 pt-5 mt-5">
          <div className="flex flex-col md:flex-row gap-5 mb-5">
            <div 
              className={`flex-1 border border-dashed rounded-lg p-5 flex flex-col items-center justify-center cursor-pointer transition-all duration-200 ${fileError ? 'border-red-300 bg-red-50' : certificateFile ? 'border-green-300 bg-green-50' : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50/50'}`}
            ...
            >
              ...
              {!certificateFile ? (
                ...
                <>
                  ...
                  {verificationStatus && (
                    <div className={`mt-3 flex items-center text-sm ${verificationStatus.isValid ? 'text-green-500' : 'text-red-500'}`}>
